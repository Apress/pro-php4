...
class webmail 
{
	...
	function geMailbox_list($ref='', $return_raw=0) 
        {
	...
	function createMailbox() 
	    ...
	    if($this->protocol == 'nntp' || $this->protocol == 'pop3' || $this->new_mailbox == 'INBOX') {
	        $this->buildErrorMsg($this->ERR_STR_CANT_CREATE_MAILBOX, $this->new_mailbox);
		return false;
	    }
	...
	function deleteMailbox() 
	    ...
	    if(!@imap_deletemailbox($this->stream, "{\$this->host}$this->del_mailbox")) {
	        $this->buildErrorMsg($this->ERR_STR_CANT_DELETE_MAILBOX, imap_last_error());		return false;     
	    }
